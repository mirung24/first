...
$(document).ready(function() {
	...
	dt1(chaincode, pageno, pagecnt);//데이터 테이블 리스트 조회
//	prod_tt(chaincode, sd1, sd2);// 상단 집계 조회
	...
	$(document).on('click','#searchBtn', function() {
		...
		dt1(chaincode, 1, pagecnt);//데이터 테이블 리스트 조회
//		prod_tt(chaincode, sd1, sd2);// 상단 집계 재조회
	});// 검색 버튼 
...
//					$('#d_prodcate').val(result.category3cd);
	...
	var table = $("#table").DataTable({
		 ...
		 ajax: {
			 ...
			 dataSrc: function(result) {
				 console.log('체인점 상품관리 리스트', result.data);
...
//				{ data: "maprice" }, // 최다가9
				{ data: "total" }, // 매출(당일)7
				{ data: "total2" }, // 매출(전일)8
			 	{ data: "total3" }, // 매출(D-30)9
				{ data: "ranking" }, // 순위(D-30)10
			 ...
			 columnDefs: [
				...
				{ targets: [4,11], render: function(data, type, row) {
...
function cateNames(category3cd) {
	...
	$.ajax({
		...
		success: function(result) {
			...
			$("#d_prodcate").val(cate1+" >  "+cate2+" >  "+cate3);// 이름으로 집어넣기
...
$(".pop2 .btn-save").click(function(){ // 유효성 검사, 주소는 검색할 수 있게 api사용
	...
	if($("#productname").val().length == 0)
		...
		swal({
			...
			var category = $('#category3cd').val();// 마지막 3번째 카테고리만
					...
					success: function(result) {
						...
						if(result == 1) {
							...
							swal({
								...
								location.href = "/product.do?title=상품관리&name=상품관리";
							});
						} 
						else if(result == 3) {
...
//입력 못하게 하기
//$("input:text[writeNo]").on("keyup", function() {
//	$(this).val(replace(/[^a-z0-9:\-_]/gi,'',""));
//});
...
$("#catesv").click(function() {
	...
	if(cateselyn > 0) {// 3차 카테고리까지 선택한 경우
		if(whichclick == "modal") { // 상품등록하는 경우
			$(".category").val(selcate1 + selcate2 + selcate3);
		} else if(whichclick == "tr") {
			$("#d_prodcate").val(selcate1 + selcate2 + selcate3);
		}
		$(".modal-wrap4").css({"display": "none"});	
