...
$(document).ready(function() {
	var table = $("#table").DataTable({
	 ...
	 ajax: { url: "../json/coupon.json"}, 
			 columns: [
				{ data: "no" },
				{ data: "couponname" },
				{ data: "price" },
				{data: "img",
				 'render': function(/* data, type, full, meta */) {return '<span class="couponimg">등록</span>';}},
				{ data: "date" },
				{ data: "use" }
			 ],
				 "language": {
...
$(".pop-close1").click(function(){
 ...
 $(".pop2 .btn-save").click(function(){
	...
	swal({title: "쿠폰명을 입력하세요.", button: "확인"})
	.then(function() {swal.close(); $(".couponname").focus(); return false;
})
	else if($(".price").val().length==0)
	swal({title: "쿠폰금액을 입력하세요.", button: "확인"})
	.then(function() {swal.close(); $(".price").focus(); return false;
})

	else if($(".date").val().length==0)
	swal({title: "유효기간을 입력하세요.", button: "확인"})
	.then(function() {swal.close(); $(".date").focus(); return false;
})
